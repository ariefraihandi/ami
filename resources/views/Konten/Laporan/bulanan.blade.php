          ...
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <div class="time-spending-chart">
                ...
                <span class="badge bg-label-success">Bulan Ini</span>
              </div>
            </div>
      ...
      <div class="col-12 col-xl-8">
        <div class="card h-100">
          <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="card-title m-0 me-2">Chart Keuangan Bulanan</h5>          
          </div>
        ...
        <div class="card h-100">
          ...
          <div class="card-body">
            <div class="mb-3">
                ...
                <select id="bulan" class="form-select" onchange="updateUrl()">
                    <?php
                    setlocale(LC_TIME, 'id_ID.utf8'); // Set locale untuk menggunakan bahasa Indonesia
                    
                    for ($i = 1; $i <= 12; $i++) {
                        // Menggunakan Carbon untuk menentukan tanggal awal dan akhir bulan
                        $startDate = \Carbon\Carbon::createFromDate(date('Y'), $i, 1)->toDateString();
                        $endDate = \Carbon\Carbon::createFromDate(date('Y'), $i, 1)->endOfMonth()->toDateString();
                        $monthName = strftime('%B', mktime(0, 0, 0, $i, 1)); // Format bulan dalam bahasa Indonesia
                        $selected = $i == $selectedMonth ? 'selected' : ''; // Tambahkan atribut selected jika bulan cocok dengan bulan dari kontroller
                        echo "<option value=\"$i\" data-startDate=\"$startDate\" data-endDate=\"$endDate\" $selected>$monthName</option>";
                    }
                    ?>
                </select>
            </div>            
            <a href="{{ url('/report') }}/?startDate={{ date('Y-m-d', strtotime($dateStarUnduh)) }}&endDate={{ date('Y-m-d', strtotime($dateEndUnduh)) }}" target="_blank" class="btn btn-label-secondary d-grid w-100 mb-3">
              <span class="d-flex align-items-center justify-content-center text-nowrap">
                <i class="bx bx-download bx-fade-down bx-xs me-2"></i>
                Download Laporan
              </span>
            </a>                      
            <a href="{{ url('/laporan') }}/?startDate={{ date('Y-m-d', strtotime($today)) }}&endDate={{ date('Y-m-d', strtotime($today)) }}" target="_blank" class="btn btn-label-success d-grid w-100 mb-3">
              <span class="d-flex align-items-center justify-content-center text-nowrap">
