    ...
    <div class="card bg-transparent shadow-none border-0 my-4">
      <div class="card-body row p-0 pb-3">
        <div class="col-12 col-md-12">
          <h3>Laporan Tahun {{$selectedYear}} 📃</h3>
          ...
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <div class="time-spending-chart">
                ...
                <span class="badge bg-label-success">Tahunan</span>
              </div>
            </div>
        ...
        <div class="card h-100">
          ...
          <div class="card-body">
            <div class="mb-3">
                ...
                <select id="tahun" class="form-select" onchange="updateUrl()">
                    <?php
                    $currentYear = date('Y');
                    $selectedYear = $selectedYear ?? $currentYear; // Mendapatkan tahun terpilih, atau menggunakan tahun saat ini jika tidak ada yang dipilih
                    $yearsToShow = 5; // Jumlah tahun sebelum dan sesudah yang akan ditampilkan
            
                    for ($i = $currentYear - $yearsToShow; $i <= $currentYear + $yearsToShow; $i++) {
                        $startDate = \Carbon\Carbon::createFromDate($i, 1, 1)->toDateString();
                        $endDate = \Carbon\Carbon::createFromDate($i, 12, 31)->toDateString();
                        $selected = $i == $selectedYear ? 'selected' : '';
                        echo "<option value=\"$i\" data-startDate=\"$startDate\" data-endDate=\"$endDate\" $selected>$i</option>";
                    }
                    ?>
                </select>
            </div>                      
            ...
            <a href="{{ url('/laporan/bulanan') }}/?startDate={{ date('Y-m-d', strtotime($startOfMonth)) }}&endDate={{ date('Y-m-d', strtotime($endOfMonth)) }}" target="_blank" class="btn btn-label-primary d-grid w-100 mb-3">
              <span class="d-flex align-items-center justify-content-center text-nowrap">
...
<script>
    ...
    <script src="{{ asset('assets') }}/js/laporan-tahunan.js"></script>
    <script>         
        @if(session('response'))
